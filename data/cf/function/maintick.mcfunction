execute as @a[tag=!edit] store result score @s menu_click run clear @s #cf:ui_items[custom_data~{menu_ui:1}] 0
execute as @a[scores={menu_click=1..}] at @s run function cf:menu/menu_click
execute as @e[type=item] if data entity @s Item.components."minecraft:custom_data".menu_ui at @s run function cf:menu/menu_drop
